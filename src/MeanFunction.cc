...
WallMean::WallMean()
{
  this->SetParams("49 3 0 0 0 0");
  this->param_dim = param.n_elem;
  this->grad_mult.set_size(param.n_elem);
  this->grad_mult.fill(1.0);
}

WallMean::WallMean(const Col<REAL>& param)
{
  this->SetParams(param);
  this->param_dim = param.n_elem;
  this->grad_mult.set_size(param.n_elem);
...
void WallMean::SetParams(const Col<REAL>& param)
{
  if (param.n_elem < 4 || (param.n_elem % 2) != 0) {
    throw std::exception();
  }
  this->param = param;
  source.set_size(2);
...
REAL WallMean::Eval(const Col<REAL>& x)
{
  REAL dist = norm(source-x, 2);
  double val;
  if(dist < 1e-6)
    ...
    val = -param(0) - 10*param(1)*log10(dist) ;

  for (int i = 4; i < param.n_elem; i+=2) {
    val += param(i) * param(i + 1);
  }
  return val;
}

void WallMean::Grad(Col<REAL> &grad, const Col<REAL>& x)
{
  throw std::exception();
}

void WallMean::Hessian(Mat<REAL> &H, const Col<REAL>& x)
{
  throw std::exception();
}


void WallMean::GradX(Col<REAL> &grad, const Col<REAL>& x)
{
  throw std::exception();
}
